      ...
      try {
        ...
        const postsPromises = savedPostIds.map(async (id: string) => {
          try {
            ...
            return response.advert as Advert; // Type assertion to ensure Advert type
          } catch (error) {
        ...
        setSavedPosts(posts.filter((post): post is Advert => post !== null));
      } catch (error: any) {
        toast({
  ...
  if (savedPosts.length === 0) {
    return (
      <div className={`text-center py-12 ${bgColor} rounded-lg backdrop-blur-sm`}>
        ...
        <p className={`${secondaryTextColor}`}>{t("upvotePosts")}</p>
      </div>
    );
  }
