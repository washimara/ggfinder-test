...
import { getAdvertById, deleteAdvert, trackAdvertView } from "@/api/adverts"
import { useToast } from "@/hooks/useToast"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { Trash2, Edit, Calendar, MapPin, Lock } from "lucide-react"
...
export function AdvertDetailPage() {
  ...
  useEffect(() => {
    const fetchAdvert = async () => {
      ...
      try {
        ...
        await trackAdvertView(id)
      } catch (error: any) {
        // Update error handling to check for the specific requiresKey flag
        if (error.message === 'This is a private advert that requires a valid access key' || 
            error.response?.data?.requiresKey) {
          setNeedsKey(true)
          if (key) {
            toast({
  ...
  const handleAccessKeySubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (accessKey) {
      // Update URL with the access key and reload the page
      navigate(`/advert/${id}?key=${accessKey}`)
    }
  }
  ...
  if (needsKey) {
    return (
      <div className="py-12 max-w-md mx-auto text-center">
        <div className={`rounded-lg ${bgColor} p-8 backdrop-blur-sm`}>
          <Lock className={`mx-auto h-12 w-12 mb-4 ${secondaryTextColor}`} />
          <h1 className={`text-2xl font-bold mb-4 ${textColor}`}>Private Advert</h1>
          <p className={`mb-6 ${secondaryTextColor}`}>
            This advert is private and requires an access key to view.
          </p>
          
          <form onSubmit={handleAccessKeySubmit} className="space-y-4">
            <div className="space-y-2 text-left">
              <Label htmlFor="access-key" className={textColor}>Access Key</Label>
              <Input 
                id="access-key"
                value={accessKey}
            ...
            <Button 
              type="submit" 
              className={`w-full ${currentTheme.buttonPrimary}`}
            >
              Access Advert
            </Button>
          </form>
  ...
  if (!advert) {
    return (
      <div className="py-12 max-w-4xl mx-auto text-center">
        <h1 className={`text-2xl font-bold mb-4 ${textColor}`}>Advert not found</h1>
        ...
        {isOwner && (
          <div className="flex gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate(`/edit-advert/${advert._id}`)}
            >
